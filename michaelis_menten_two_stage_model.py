...
def simulate_two_stage_Michaelis_Menten_ode(y0, D, B, Vmax, km, k, dose_times, end_time):
    t = np.array([])
    b = np.array([])
    s = np.array([])
...
T12 = 1.5
k = np.log(2)/T12
dose_times = [6,12,18,24]
end_time = 48
 
#t,b,s = simulate_two_stage_Michaelis_Menten_ode(y0, D, B, Vmax, km, k, dose_times, end_time)
t, b, s = simulate_two_stage_Michaelis_Menten_ode(y0, D, B, Vmax, km, k, dose_times, end_time)
